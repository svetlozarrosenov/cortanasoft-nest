...
exports.ErrorMessages = {
    common: {
        ...
        missingPermission: (permission) => `Достъпът е отказан. Нямате право за: ${permission}`,
    ...
    products: {
        ...
        skuExists: (sku) => `Продукт с артикулен номер "${sku}" вече съществува`,
        categoryNotFound: 'Категорията не е намерена',
        categoryExists: (name) => `Категория "${name}" вече съществува`,
    ...
    suppliers: {
        ...
        eikExists: (eik) => `Доставчик с ЕИК "${eik}" вече съществува`,
    ...
    locations: {
        ...
        codeExists: (code) => `Локация с код "${code}" вече съществува`,
        cannotDeleteWithInventory: 'Не може да изтриете локация с налични продукти',
        zoneCodeExists: (code) => `Зона с код "${code}" вече съществува в тази локация`,
...
function getPrismaErrorMessage(error) {
    if (error.code === 'P2002') {
        ...
        return exports.ErrorMessages.database.uniqueViolation;
    }
    if (error.code === 'P2003') {
        return exports.ErrorMessages.database.foreignKeyViolation;
    }
    if (error.code === 'P2025') {
        return exports.ErrorMessages.database.recordNotFound;
    }
    if (error.code === 'P2014') {
