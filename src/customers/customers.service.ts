  ...
  async update(
    ...
    if (existing.isPartner && nextIsPartner === false) {
      ...
      if (partnerAccountsCount > 0) {
        throw new BadRequestException(
          'Клиентът не може да спре да бъде партньор — има асоцииран партньорски потребителски акаунт. Свържете се с администратора си, за да бъде премахната асоциацията.',
        );
      }
    }
